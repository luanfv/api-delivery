...
import { StartDeliveryController } from './StartDeliveryController';
import { prisma } from '../../../../database/prismaClient';

const app = express();

describe('src/modules/deliveryman/useCases/startDelivery/StartDeliveryController', () => {
  const request = app.request;
  const response = app.response;
  const startDeliveryController = new StartDeliveryController();

  describe('when try start delivery', () => {
    describe('and don\'t receive "id_deliveryman"', () => {
      ...
      it('should throw an exception with message "Didn\'t receive the deliveryman\'s identify"', async () => {
        await expect(
          async () => await startDeliveryController
      ...
      it('should throw an exception with cause 401', async () => {
        ...
        const result = await startDeliveryController
          .handle(request, response)
    ...
    describe('and manages to start', () => {
      ...
      it('should return delivery', async () => {
        const result = await startDeliveryController.handle(request, response);
